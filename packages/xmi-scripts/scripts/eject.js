...
inquirer
  ...
  .then(answer => {
    ...
    if (appPackage.devDependencies) {
      // We used to put xmi-scripts in devDependencies
    ...
    if (fs.existsSync(paths.appTypeDeclarations)) {
      try {
        ...
        // Remove xmi-scripts reference since they're getting a copy of the types in their project
        content =
          content
            // Remove xmi-scripts types
            .replace(
              /^\s*\/\/\/\s*<reference\s+types.+?"xmi-scripts".*\/>.*(?:\n|$)/gm,
              ''
            )
            .trim() + os.EOL;
    ...
    if (ownPath.indexOf(appPath) === 0) {
      try {
        // remove xmi-scripts and xmi-scripts binaries from app node_modules
        Object.keys(ownPackage.bin).forEach(binKey => {
    ...
    if (fs.existsSync(paths.yarnLockFile)) {
      ...
      if (process.platform === 'win32') {
        // https://github.com/echo008/xmi/pull/3806#issuecomment-357781035
        // Yarn is diligent about cleaning up after itself, but this causes the xmi-scripts.cmd file
