# xmi-docgen [![Build Status](https://travis-ci.org/echo008/xmi-docgen.svg?branch=master)](https://travis-ci.org/echo008/xmi-docgen)

`xmi-docgen` is a CLI and toolbox to help extracting information from [React][] components, and generate documentation from it.
...
> xmi-docgen is a low level tool to extract information about react components. If you are searching for a more high level tool try [react-styleguidist](https://github.com/styleguidist/react-styleguidist) which comes with a complete styleguide and is build on top of xmi-docgen.
...
```
yarn add xmi-docgen --dev
```

```
npm install --save-dev xmi-docgen
```
...
Installing the module adds a `xmi-docgen` executable which allows you to convert
...
By default, `xmi-docgen` will look for the exported component created through
...
```js
var xmiDocs = require('xmi-docgen');
var componentInfo = xmiDocs.parse(src);
```
...
| options      | Object | Pass options to xmi-docgen. Supported option is `legacyDecorators` which is a boolean |
...
> There are some community created handlers available. Have a look at the wiki for a list: https://github.com/echo008/xmi-docgen/wiki
...
If you are using [flow][flow] then xmi-docgen can also extract the flow type annotations. As flow has a way more advanced and fine granular type system, the returned types from xmi-docgen are different in comparison when using `React.PropTypes`.

> **Note**: xmi-docgen will not be able to grab the type definition if the type is imported or declared in a different file.
